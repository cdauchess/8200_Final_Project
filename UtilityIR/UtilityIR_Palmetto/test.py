...
#/project/bli4/maps/CD_RC_SL_8200Project/8200_Final_Project/datasets/GetNuScImage.py
...
parser.add_argument("--testing", type=str, default=r'test_image', help="testing function. All list: ['test_image', 'test_plot_latent_tsne', 'test_latent_manipulation', 'eval_objective', 'test_QA', 'NS_INFERENCE']")
...
parser.add_argument("--model", type=int, default=1, help="1 or 2")
...
def convNSImage(epoch, dataRoot, input_list, NSDL, modelUsed):
    ...
    val_dataloader = DataLoader(
        NSTestDataset(transforms_=transforms_test, imageList=input_list),
        batch_size=1,
        shuffle=False,
        num_workers=0,
...
if __name__ == '__main__':
    ...
    if cuda:
        if opt.model == 1:
            print("Loading Setting 1")
            utilityIR.load_state_dict(torch.load("saved_models/%s/utilityIR_setting1.pth" % (opt.exp_name)))
        elif opt.model ==2:
            print("Loading Setting 2")
            utilityIR.load_state_dict(torch.load("saved_models/%s/utilityIR_setting2.pth" % (opt.exp_name)))
    else:
        print('NOT USING CUDA - Loading "BEST"')
    ...
    elif testing == 'NS_INFERENCE':
        DL = NS_ImageLoader(dataRoot, 'val')
        inputList = DL.SingleListImages(onlyRain=False)
        convNSImage(epoch=opt.checkpoint, dataRoot=dataRoot, input_list = inputList, NSDL = DL, modelUsed = opt.model) #Convert the images
